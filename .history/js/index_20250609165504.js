...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      // 2) 클릭된 버튼에만 활성 클래스 추가
      this.classList.remove("text-gray-400", "border-gray-400");
      this.classList.add("text-teal-400", "border-teal-400");
      this.setAttribute("aria-selected", "true");

      // ─────────────────────────────────────────────
      // 3) 모든 패널을 숨기고 (hidden 추가)
      panels.forEach(panel => panel.classList.add("hidden"));

      // 4) 클릭된 탭의 data-tabs-target이 가리키는 패널만 보이기
      const targetSelector = this.getAttribute("data-tabs-target"); // ex) "#abroad"
