...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tabButtons.forEach(btn => {
    btn.addEventListener("click", function () {
      ...
      this.classList.replace("text-gray-400","text-teal-400");
      this.classList.replace("border-gray-400","border-teal-400");
      this.setAttribute("aria-selected", "true");

      // ── 패널 토글 ──
      // 1) 모든 패널 숨기기
      panels.forEach(panel => panel.classList.add("hidden"));
      // 2) 클릭된 버튼의 data-tabs-target으로 가리키는 패널만 보이기
      const target = this.getAttribute("data-tabs-target"); // e.g. "#abroad"
